...
export async function registerRoutes(
  ...
  app.get("/api/strategies", async (req, res) => {
    ...
    const result = [
      ...
      formatBucket(obosBucket, "obos", "RSI-26 Core-4 (5m+15m AND)", openObos),
  ...
  app.post("/api/trades/sync-from-hl", async (_req, res) => {
    try {
      ...
      broadcast({ type: "status", data: await tradingEngine.getStatus() });
      res.json({ success: true, synced, message: `Imported ${synced.length} position(s) from Hyperliquid (v15.4: RSI-26 Core-4 + Oil)` });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  });
  ...
  app.get("/api/equity-curve", async (_req, res) => {
    try {
      ...
      const baselineTs = config?.pnlBaselineTimestamp || V15_START;
