...
export async function registerRoutes(
  ...
  app.get("/api/strategies", async (req, res) => {
    ...
    const result = [
      ...
      formatBucket(obosBucket, "obos", "RSI-26 Multi (Top-10)", openObos),
  ...
  app.post("/api/trades/sync-from-hl", async (_req, res) => {
    try {
      ...
      broadcast({ type: "status", data: await tradingEngine.getStatus() });
      res.json({ success: true, synced, message: `Imported ${synced.length} position(s) from Hyperliquid (v15.3: RSI-26 Multi + Oil)` });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  });
