...
export async function initDatabase() {
  ...
  try {
    ...
    const alterQueries = [
      ...
      // v17.4.1 (ISSUE-010): persist session state across restarts so firstDecisionAttempted flag
      // and retryHistory survive deploys. Without this column, state was silently dropped every restart.
      "ALTER TABLE bot_config ADD COLUMN IF NOT EXISTS session_state TEXT DEFAULT ''",
    ];
